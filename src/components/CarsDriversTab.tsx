...
export const CarsDriversTab: React.FC = () => {
  ...
  const [ownerCompany, setOwnerCompany] = useState('مجموعة البنا للتشييد');
    ...
    try {
      if (importTarget === 'cars') {
        ...
        lines.forEach((line, idx) => {
          ...
          if (!targetDriver) {
            duplicateWarnings += ` • السائق صاحب الاسم [${drvName}] غير مسجل، تم تخطي المخالفة للسيارة ${carNum}.\n`;
            failCount++;
          } else {
            ...
            if (res.success) {
              ...
              duplicateWarnings += ` • مخالفة مكررة للسيارة ${carNum} بتاريخ ${vDate} (تخطي).\n`;
              failCount++;
            }
          }
        });
        ...
        lines.forEach((line, idx) => {
          ...
          const dDate = cols[2]?.trim() || new Date().toISOString().split('T')[0];
          ...
          <div className="lg:col-span-4 bg-white p-5 rounded-2xl border border-slate-100 shadow-sm space-y-4">
            ...
            <form onSubmit={handleSaveCar} className="space-y-3.5 text-xs">
              ...
              <div>
                ...
                >
                  <option value="مجموعة البنا للتشييد">مجموعة البنا للتشييد</option>
                  <option value="البنا ترانس لخدمات اللوجستيك">البنا ترانس لخدمات اللوجستيك</option>
                  <option value="البنا الخرسانية">البنا الخرسانية</option>
                </select>
              </div>
