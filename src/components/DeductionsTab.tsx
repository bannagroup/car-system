...
export const DeductionsTab: React.FC = () => {
  ...
  const handleSaveRowDeduction = (driver: Driver) => {
    ...
    // Apply deduction
    db.applyIndividualDeduction(driver.id, amt, notes);
  ...
  const handleApplyIndividual = (e: React.FormEvent) => {
    ...
    db.applyIndividualDeduction(selectedDriverId, Number(individualAmount), "خصم مستقطع");

    // Open Printer A5 Preview Immediately
    setPrintDocument({
      type: 'deduction',
      driver: { ...targetDriver, balance: Math.max(0, targetDriver.balance - Number(individualAmount)) }, // calculated preview balance
      amount: Number(individualAmount),
      description: "خصم مستقطع"
    });
            ...
            <table className="w-full text-right border-collapse min-w-[700px]">
              <thead>
                <tr className="bg-slate-50 text-slate-500 border-b border-slate-100">
                  ...
                  <th className="py-2.5 px-3 text-right" style={{ width: '130px' }}>تسجيل الخصم</th>
                      ...
                      <tr key={d.id} className="border-b border-slate-100 hover:bg-slate-50/80 transition-colors">
                        ...
                        <td className="py-3 px-3">
                          <input
                            ...
                            className="w-full p-2 border border-slate-250 rounded text-xs font-mono font-bold focus:outline-none focus:border-indigo-500 bg-white"
                          />
                        </td>
